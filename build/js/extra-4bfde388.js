...
function c(e, t, n=e.target.href) {
    function o(e) {
        ...
        t && (t.style.colorScheme = document.documentElement.style.colorScheme,
        t.style.color = window.getComputedStyle(document.documentElement).getPropertyValue("color"))
    }
    if (screen.width * screen.height < 614400 || !navigator.onLine || e.altKey || e.ctrlKey || e.shiftKey || e.metaKey || e?.button > 0)
        return;
    e.preventDefault(),
    i(e);
    ...
    a.addEventListener("transitionend", (function(e) {
        "top" === e.propertyName && (this.classList.add("ExpandDone"),
        o({
            target: r
        }))
    }
...
o("CloseUp").querySelector("a").addEventListener("click", (e=>{
    let t;
    o("CloseUp").classList.contains("Tope") && (t = e.target.href.replace("//en.", "//isotopes.")),
    c(e, o("CloseUp"), t)
}
)),
o("Series").querySelectorAll("a").forEach((e=>e.addEventListener("click", (t=>c(t, e.closest("dt, dd")))))),
o("StateOfMatter").querySelectorAll("a").forEach((e=>e.addEventListener("click", (t=>c(t, e.closest("div")))))),
o("DecayModes").querySelectorAll("a").forEach((e=>e.addEventListener("click", (t=>c(t, e.closest("div")))))),
...
o("Ptable").querySelectorAll(".Group > a").forEach((e=>e.addEventListener("click", (t=>c(t, e.closest(".Group")))))),
...
async function g(e) {
    function t(e) {
        ...
        this.querySelectorAll("a, input, select, [tabindex]").forEach((t=>t.tabIndex = e ? 0 : -1)),
        this.querySelectorAll("select").forEach((e=>e.disabled = !Boolean(e.options.length - 1)))
    }
    await p(),
    e.forEach((e=>t.call(e.target, e.isIntersecting)))
}
function f(e, t) {
...
!async function() {
    await p(),
    "IntersectionObserver"in window && (f(h("KeyRegion"), g),
    f(h("DataRegion"), g))
}();
...
async function k() {
    ...
    let i, s, r;
    if (e.atomic_map.has(Number(o)))
        i = e.atomic_map.get(Number(o));
    else {
        let e = [...c].filter((([e,t])=>e.children[0].textContent.toLowerCase() === o));
        e.length && (i = e[0][0])
    }
    "-" === o.charAt(0) ? (r = "~",
    s = Number(o)) : [">", "<", "=", "~"].includes(o.charAt(0)) ? (r = o.charAt(0),
    s = Number(o.substring(1))) : o.substring(1).includes("-") || o.includes("..") ? (r = "-",
    s = o.split(/-|\.\./)) : isNaN(o) ? (r = "S",
    s = o.toLowerCase()) : (r = "~",
    s = Number(o)),
    ...
    if ("=" === r)
        ...
        l = [...c].filter((([e,t])=>Number(t) < s));
    else if (">" === r)
        l = [...c].filter((([e,t])=>Number(t) > s));
    else if ("-" === r && 2 === s.length)
        l = [...c].filter((([e,t])=>t >= Number(s[0] || Number.MIN_VALUE) && t <= Number(s[1] || Number.MAX_VALUE)));
    else if ("~" === r) {
        let e = [...c.values()].filter((e=>null != e)).reduce(((e,t)=>Math.min(Math.abs(t - s), e)), Number.MAX_VALUE);
        l = [...c].filter((([t,n])=>Math.abs(n - s) <= e))
    } else
        "S" === r && (l = [...c].filter((([e,t])=>String(t).toLowerCase().includes(s))));
    ...
    l = l.map((e=>e[0])),
    l.length ? i && (l[0] !== i || l.length > 1) ? ("atomic" === n && (l = [i]),
    v("SearchType").classList.add("Ambiguous")) : v("SearchType").classList.remove("Ambiguous") : (i && l.push(i),
    v("SearchType").classList.remove("Ambiguous")),
    function(t) {
        document.querySelectorAll("ol > li").forEach((e=>e.classList.toggle("Dim", !t.includes(e)))),
        1 === t.length && (t[0].dispatchEvent(e.hoverElement),
        t[0].scrollIntoView(!1))
    }(l)
}
v("SearchButton").addEventListener("click", (e=>e.currentTarget.checked ? (document.addEventListener("keydown", w),
...
E.dispatchEvent(new Event("input")),
void v("SearchConsole").addEventListener("transitionend", b)) : L())),
...
N("Blocks").querySelectorAll("dt").forEach((e=>e.addEventListener("pointerenter", C))),
...
function x(e) {
    B(),
    I("Ptable").classList.add(e.split(" ")[0])
}
...
function W() {
    ...
    this.dataset.classes ? D("Ptable").classList.add(...this.dataset.classes.split(" ")) : D("Ptable").classList.add(...this.classList)
}
...
function O(t) {
    ...
    t.isTrusted && localStorage.setItem("Theme", this.checked ? "Dark" : "Light");
...
$("DarkButton").checked = $("DarkButtonFooter").checked = document.documentElement.classList.contains("Dark");
const F = e=>document.getElementById(e);
let H;
function U(e) {
    e.key && "Enter" !== e.key || (F("Wide").checked = !0,
    j.call(F("Wide"), {
        isTrusted: !0
    }))
}
...
async function z(e) {
    ...
    t >= 0 && t <= 15 && (await new Promise(requestAnimationFrame),
    document.documentElement.dataset.wide = t,
    document.documentElement.style.setProperty("--groups", Math.min(32, t + 18)),
...
!function() {
    ...
    document.querySelectorAll(".Label57to71, .Label89to103").forEach((e=>e.addEventListener("keypress", U)));
    const t = Array(15).fill().map(((e,t)=>t + 1));
    for (let n = 0; n < 15; n++)
...
function X(e) {
    e.matches && V("SidebarToggle").checked && (document.documentElement.classList.toggle("Sidebar", !1),
...
function J() {
    ...
    document.documentElement.offsetHeight,
    document.documentElement.classList.contains("Reset") ? G() : (document.documentElement.classList.add("SmoothResize", "Collapse"),
    K.addEventListener("transitionend", Q))
}
function G() {
    document.documentElement.classList.toggle("Sidebar", V("SidebarToggle").checked),
    localStorage.setItem("Layout", V("SidebarToggle").checked ? "Sidebar" : "Topbar")
}
function Q(e) {
    e.target === this && e.propertyName.startsWith("flex") && (this.removeEventListener("transitionend", Q),
    G(),
    document.documentElement.classList.remove("Collapse"),
    K.addEventListener("transitionend", Z))
}
function Z(e) {
    e.target === this && e.propertyName.startsWith("flex") && (this.removeEventListener("transitionend", Z),
...
function ne() {
    ...
    e.src = "https://pagead2.googlesyndication.com/pagead/js/adsbygoogle.js?client=ca-pub-1899956715581914",
...
!function() {
    let e = !0;
    window.Windows && (e = !1),
    document.monetization && ("started" === document.monetization.state ? (e = !1,
    oe(100)) : document.monetization.addEventListener("monetizationstart", (()=>oe(100))));
    const t = (n = "PatreonSubscription",
...
function ie(e) {
    ...
    caches.open(window.CACHE_NAME).then((e=>e.addAll(t))).catch(reportError)
}
Array.from(e.atomic_map.values()).pop().addEventListener("animationend", (e=>{
    "shine" === e.animationName && ce()
}
)),
document.querySelector("#ResetTab + label").addEventListener("click", (()=>{
    document.documentElement.classList.contains("Reset") && (function() {
        const t = function(e) {
            const t = [];
...
function se(e) {
    ...
    e.preventDefault(),
    navigator.share && navigator.share(t).catch(reportError)
}
function re(e) {
    const t = (t,n,o)=>{
        !function(e, t, n) {
    ...
    return {
        ...
        enableAutoPageviews: ()=>{
            const t = ()=>n()
              , o = history.pushState;
            return o && (history.pushState = function(e, n, a) {
                o.apply(this, [e, n, a]),
                t()
            }
            ,
        ...
        enableAutoOutboundTracking: (e=document,n={
            ...
            function o(e) {
                ...
                "undefined" != typeof process && process && "test" === process.env.NODE_ENV || setTimeout((()=>{
            ...
            const a = new Set;
            function c(e) {
                e instanceof HTMLAnchorElement ? e.host !== location.host && (e.addEventListener("click", o),
                a.add(e)) : "querySelectorAll"in e && e.querySelectorAll("a").forEach(c)
            }
            function i(e) {
                e instanceof HTMLAnchorElement ? (e.removeEventListener("click", o),
                a.delete(e)) : "querySelectorAll"in e && e.querySelectorAll("a").forEach(i)
            }
            const s = new MutationObserver((e=>{
                e.forEach((e=>{
                    "attributes" === e.type ? (i(e.target),
                    c(e.target)) : "childList" === e.type && (e.addedNodes.forEach(c),
                    e.removedNodes.forEach(i))
                }
                ))
            }
            ));
            return e.querySelectorAll("a").forEach(c),
            s.observe(e, n),
            function() {
                a.forEach((e=>{
...
"complete" === document.readyState ? le() : window.addEventListener("load", le);
function de() {
    return "requestIdleCallback"in window ? new Promise(requestIdleCallback) : new Promise((e=>setTimeout(e, 1)))
}
function ue() {
...
async function pe(e) {
    ...
    await ue(),
    e.forEach(((e,t)=>he.call(e.children[2], n[t], o[t])))
}
